        ...
        {weekDates.map((date, i) => {
          ...
          return (
            <div
              key={i}
              className={cn(
                "flex flex-col items-center gap-2 rounded-md p-2 text-center transition-all duration-300",
                isToday
                  ? "rounded-md border border-lime-600 font-bold text-lime-800"
                  : "text-lime-700",
              )}
              style={{ flexBasis: "14%" }}
            >
              <p className="text-xs font-medium uppercase">{dayLetters[i]}</p>
